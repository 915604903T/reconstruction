...
try
{
  ...
  // Parse the command-line arguments. (set headless mode as default)
  ...
  if(args.host != "")
  {
    ...
    const pooled_queue::PoolEmptyStrategy poolEmptyStrategy =
        settings->get_first_value<pooled_queue::PoolEmptyStrategy>("MappingClient.poolEmptyStrategy",
                                                                   pooled_queue::PES_DISCARD);
    pipeline->set_mapping_client(Model::get_world_scene_id(),
                                 MappingClient_Ptr(new MappingClient(args.host, args.port, poolEmptyStrategy)));
  }
  ...
  Application app(pipeline, args.renderFiducials, args.name);
