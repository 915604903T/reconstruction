...
// Note: This must appear before anything that could include SDL.h, since it includes boost/asio.hpp, a header that has
// a WinSock conflict with SDL.h.
...
/**
 * \brief Copies any (voxel) scene parameters that have been specified in the configuration file across to the actual
...
{
#define COPY_PARAM(type, name, defaultValue)                                                                           \
  settings->sceneParams.name = settings->get_first_value<type>("SceneParams." #name, defaultValue)
...
/**
 * \brief Copies any surfel scene parameters that have been specified in the configuration file across to the actual
 * surfel scene parameters object.
 *
 * \param settings  The settings for the application.
 */
...
{
#define COPY_PARAM(type, name, defaultValue)                                                                           \
  settings->surfelSceneParams.name = settings->get_first_value<type>("SurfelSceneParams." #name, defaultValue)
...
/**
 * \brief Determines the sequences (if any) that the user wants to load from disk, based on the program's command-line
 * arguments.
 *
...
std::vector<Sequence_CPtr> determine_sequences(const CommandLineArguments &args)
{
  ...
  for(size_t i = 0, size = args.sequenceSpecifiers.size(); i < size; ++i)
  {
    ...
    if(!bf::is_regular_file(sequenceSpecifier))
    {
      ...
      sequences.push_back(
          Sequence_CPtr(new SpaintSequence(dir, initialFrameNumber, missingDepthFraction, depthNoiseSigma)));
    }
    else
      throw std::runtime_error("Error: The sequence specifier '" + sequenceSpecifier +
                               "' denotes a file rather than a directory");
  }
  ...
  for(size_t i = 0, size = args.depthImageMasks.size(); i < size; ++i)
  {
    ...
    sequences.push_back(Sequence_CPtr(new SpaintSequence(depthImageMask,
                                                         rgbImageMask,
                                                         poseFileMask,
                                                         semanticImageMask,
                                                         initialFrameNumber,
                                                         missingDepthFraction,
                                                         depthNoiseSigma)));
  }
...
std::map<std::string, DualQuatd> load_global_poses(const std::string &globalPosesSpecifier)
{
  ...
  const bf::path p = bf::is_regular(globalPosesSpecifier)
                         ? globalPosesSpecifier
                         : find_subdir_from_executable(dirName) / (globalPosesSpecifier + ".txt");
  ...
  std::cout << "[Spaint]: globalPose: " << p.string() << "\n";
  std::string id;
  DualQuatd dq;
  while(fs >> id >> dq)
  {
...
ImageSourceEngine *make_camera_subengine(const CommandLineArguments &args)
{
  ...
  if(cameraSubengine == NULL)
  {
    ...
    boost::optional<std::string> uri =
        args.openNIDeviceURI == "Default" ? boost::none : boost::optional<std::string>(args.openNIDeviceURI);
    bool useInternalCalibration = !uri; // if reading from a file, assume that the provided calibration is to be used
    cameraSubengine = check_camera_subengine(new OpenNIEngine(
        args.calibrationFilename.c_str(),
        uri ? uri->c_str() : NULL,
...
boost::shared_ptr<CompositeImageSourceEngine> make_image_source_engine(const CommandLineArguments &args)
{
  ...
  // If a model was specified without either a disk sequence or the camera following it, add an idle subengine to allow
  // the model to still be viewed.
  ...
  for(size_t i = 0; i < args.sequences.size(); ++i)
  {
    ...
    const std::string calibrationFilename = (args.calibrationFilename != "" || !bf::exists(calibrationPath))
                                                ? args.calibrationFilename
                                                : calibrationPath.string();

    std::cout << "[spaint] Reading images from disk: " << *args.sequences[i] << '\n';
    imageSourceEngine->addSubengine(new AsyncImageSourceEngine(
        args.sequences[i]->make_image_source_engine(calibrationFilename), args.prefetchBufferCapacity));
  }

  // If no model and no disk sequences were specified, or we want to switch to the camera once all the disk sequences
  // finish, add a camera subengine.
  if((!args.modelDir && args.sequences.empty()) || args.cameraAfterDisk)
  {
...
/**
 * \brief Makes the overall tracker configuration based on any tracker specifiers that were passed in on the command
 * line.
 *
 * \param args  The program's command-line arguments.
...
std::string make_tracker_config(const CommandLineArguments &args)
{
  ...
  {
    // Look to see if the user specified an explicit tracker specifier for it on the command line; if not, use a default
    // tracker specifier.
    ...
    for(size_t j = 0; j < chunkCount; ++j)
    {
      ...
      else if(chunks[j] == "Disk")
      {
        ...
        if(!globalPoses.empty())
        {
          ...
          {
            // FIXME: We shouldn't hard-code "Local" here - it's based on knowing how CollaborativePipeline assigns
            // scene names.
            const std::string sceneID =
                i == 0 ? Model::get_world_scene_id() : "Local" + boost::lexical_cast<std::string>(i);
          ...
          if(it != globalPoses.end())
            result += "<tracker type='global'><params>" + boost::lexical_cast<std::string>(it->second) + "</params>";
          else
            throw std::runtime_error("Error: Global pose for sequence '" + sequenceID + "' not found");
        }
 ...
 * \param settings  The settings for the application.
 */
void parse_configuration_file(const std::string &filename,
 ...
 * \param settings  The settings for the application.
 */
void postprocess_arguments(CommandLineArguments &args,
                           const po::options_description &options,
                           po::variables_map &vm,
                           const Settings_Ptr &settings)
{
  // Determine the sequences (if any) that the user wants to load from disk, based on the program's command-line
  // arguments.
  args.sequences = determine_sequences(args);

  // If the user specified a model to load, determine the model directory and parse the model's configuration file (if
  // present).
  if(args.modelSpecifier != "")
  {
    args.modelDir = bf::is_directory(args.modelSpecifier)
                        ? args.modelSpecifier
                        : find_subdir_from_executable("models") / args.modelSpecifier / Model::get_world_scene_id();
  ...
  // If the user wants to use global poses for the scenes, make sure that each disk sequence has a tracker specifier set
  // to Disk.
  ...
  // If the user wants to use a Vicon fiducial detector or a Vicon-based tracker, make sure that the Vicon system it
  // needs is enabled.
...
bool parse_command_line(int argc, char *argv[], CommandLineArguments &args, const Settings_Ptr &settings)
{
  ...
  genericOptions.add_options()("help", "produce help message")(
      "scene1", po::value<std::string>(&args.scene1), "the one scene prepare to relocalise")(
      "scene2", po::value<std::string>(&args.scene2), "the other scene prepare to relocalise")(
      ...
      "collaboration mode (batch|live)")("configFile,f", po::value<std::string>(), "additional parameters filename")(
      ...
      "fiducialDetectorType",
      po::value<std::string>(&args.fiducialDetectorType)->default_value("aruco"),
      "fiducial detector type (aruco|vicon)")("globalPosesSpecifier,g",
                                              po::value<std::string>(&args.globalPosesSpecifier)->default_value(""),
                                              "global poses specifier")(
      "headless", po::bool_switch(&args.headless)->default_value(true), "run in headless mode")(
      "host,h", po::value<std::string>(&args.host)->default_value(""), "remote mapping host")(
      "leapFiducialID",
      po::value<std::string>(&args.leapFiducialID)->default_value(""),
      "the ID of the fiducial to use for the Leap Motion")(
      "mapSurfels", po::bool_switch(&args.mapSurfels), "enable surfel mapping")(
      "missingDepthFraction",
      po::value<std::vector<double>>(&args.missingDepthFractions)->multitoken(),
      "missing depth fraction [0,1]")(
      ...
      "profileMemory",
      po::bool_switch(&args.profileMemory)->default_value(false),
      "whether or not to profile the memory usage")(
      ...
      "saveModelsOnExit",
      po::bool_switch(&args.saveModelsOnExit),
      "save a model of each voxel scene on exiting the application")(
      "subwindowConfigurationIndex",
      po::value<std::string>(&args.subwindowConfigurationIndex)->default_value("1"),
      "subwindow configuration index")("trackerSpecifier,t",
                                       po::value<std::vector<std::string>>(&args.trackerSpecifiers)->multitoken(),
                                       "tracker specifier")(
  ...
  diskSequenceOptions.add_options()(
      ...
      "initialFrame,n",
      po::value<std::vector<size_t>>(&args.initialFrameNumbers)->multitoken(),
      "initial frame numbers")("poseMask,p",
                               po::value<std::vector<std::string>>(&args.poseFileMasks)->multitoken(),
                               "pose file mask")("prefetchBufferCapacity,b",
                                                 po::value<size_t>(&args.prefetchBufferCapacity)->default_value(60),
                                                 "capacity of the prefetch buffer")(
      "rgbMask,r", po::value<std::vector<std::string>>(&args.rgbImageMasks)->multitoken(), "RGB image mask")(
      "sequenceSpecifier,s",
      po::value<std::vector<std::string>>(&args.sequenceSpecifiers)->multitoken(),
      "sequence specifier")(
  ...
  objectivePipelineOptions.add_options()("trackObject", po::bool_switch(&args.trackObject), "track the object");
...
try
{
  ...
  // Copy any scene parameters that have been set in the configuration file across to the actual scene parameters
  // objects.
  ...
  if(args.runServer)
  {
    const MappingServer::Mode mode =
        args.pipelineType == "collaborative" ? MappingServer::SM_MULTI_CLIENT : MappingServer::SM_SINGLE_CLIENT;
  ...
  /*if(args.pipelineType != "collaborative")
  {
    ...
    SLAMComponent::TrackingMode trackingMode = args.trackSurfels ? SLAMComponent::TRACK_SURFELS :
  SLAMComponent::TRACK_VOXELS;
  ...
  std::vector<CompositeImageSourceEngine_Ptr> imageSourceEngines = make_image_source_engines(args);
  ...
  /*for(size_t i = 0; i < args.sequences.size(); ++i)
  {
    ...
    const std::string calibrationFilename = bf::exists(calibrationPath) ? calibrationPath.string() :
  args.calibrationFilename;
    ...
    imageSourceEngine->addSubengine(new
  AsyncImageSourceEngine(args.sequences[i]->make_image_source_engine(calibrationFilename),
