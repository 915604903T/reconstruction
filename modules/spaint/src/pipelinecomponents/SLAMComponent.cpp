...
SLAMComponent::SLAMComponent(const SLAMContext_Ptr &context,
  ...
  , m_trackingMode(trackingMode)
{
  ...
  slamState->set_voxel_scene(SpaintVoxelScene_Ptr(new SpaintVoxelScene(
  ...
  slamState->set_tracking_controller(m_trackingController);
  ...
  slamState->set_live_voxel_render_state(VoxelRenderState_Ptr(
      ITMRenderStateFactory<ITMVoxelIndex>::CreateRenderState(trackedImageSize, voxelScene->sceneParams, memoryType)));
  if(mappingMode != MAP_VOXELS_ONLY)
  {
    slamState->set_live_surfel_render_state(SurfelRenderState_Ptr(
        new ITMSurfelRenderState(trackedImageSize, settings->surfelSceneParams.supersamplingFactor)));
  }
...
bool SLAMComponent::get_fusion_enabled() const { return m_fusionEnabled; }

const std::string &SLAMComponent::get_scene_id() const { return m_sceneID; }

TrackingController_Ptr& SLAMComponent::get_tracking_controller() { return m_trackingController; }
...
void SLAMComponent::load_models(const std::string &inputDir)
{
  ...
  // TODO: If we support surfel model loading at some point in the future, the surfel model should be loaded here as
  // well.
...
void SLAMComponent::mirror_pose_of(const std::string &mirrorSceneID) { m_mirrorSceneID = mirrorSceneID; }
...
bool SLAMComponent::process_frame()
{
  ...
  else
  {
    ...
    // If finish training is enabled and no more images are expected, let the relocaliser know that no more calls will
    // be made to its train or update functions.
    if(m_finishTrainingEnabled && inputStatus == SLAMState::IS_TERMINATED &&
  ...
  if(inputMask && inputMask->noDims == view->depth->noDims)
  {
    ...
    maskedDepthImage = SegmentationUtil::apply_mask(
        inputMask, ORFloatImage_CPtr(view->depth, boost::serialization::null_deleter()), -1.0f);
  ...
  if(runFusion)
  {
    ...
    m_denseVoxelMapper->ProcessFrame(
        view.get(), trackingState.get(), voxelScene.get(), liveVoxelRenderState.get(), resetVisibleList);
    if(m_mappingMode != MAP_VOXELS_ONLY)
    {
      m_denseSurfelMapper->ProcessFrame(
          view.get(), trackingState.get(), surfelScene.get(), liveSurfelRenderState.get());
    }
  ...
  {
    // If we're not fusing, but the tracking has not completely failed, update the list of visible blocks so that things
    // are kept up to date.
  ...
  // If we're using surfel mapping, render a supersampled index image to use when finding surfel correspondences in the
  // next frame.
  if(m_mappingMode != MAP_VOXELS_ONLY)
  {
    m_context->get_surfel_visualisation_engine()->FindSurfaceSuper(
        surfelScene.get(), trackingState->pose_d, &view->calib.intrinsics_d, USR_RENDER, liveSurfelRenderState.get());
  }

  // If we're using a composite image source engine, the current sub-engine has run out of images and we're not using
  // global poses, disable fusion.
  CompositeImageSourceEngine_CPtr compositeImageSourceEngine =
      boost::dynamic_pointer_cast<const CompositeImageSourceEngine>(m_imageSourceEngine);
  const bool usingGlobalPoses =
      m_context->get_settings()->get_first_value<std::string>("globalPosesSpecifier", "") != "";
  if(compositeImageSourceEngine && !compositeImageSourceEngine->getCurrentSubengine()->hasMoreImages() &&
     !usingGlobalPoses)
    m_fusionEnabled = false;

  // If we're using a fiducial detector and the user wants to detect fiducials and the tracking is good, try to detect
  // fiducial markers in the current view of the scene and update the current set of fiducials that we're maintaining
  // accordingly.
...
#ifdef WITH_VICON
  // If we're using a Vicon fiducial detector to calibrate the Vicon system, and a stable pose for the Vicon origin has
  // newly been determined, store the relative transformation from world space to Vicon space.
  const ViconInterface_Ptr &vicon = m_context->get_vicon();
  if(vicon && !vicon->get_world_to_vicon_transform(m_sceneID) &&
     boost::dynamic_pointer_cast<const ViconFiducialDetector>(fiducialDetector))
  {
...
bool SLAMComponent::process_frame_pose()
{
  ...
  {
    // const SLAMState::InputStatus inputStatus = m_imageSourceEngine->hasMoreImages() ? SLAMState::IS_IDLE :
  ...
  */
  ...
  {
    view->depth->UpdateHostFromDevice();
    maskedDepthImage = SegmentationUtil::apply_mask(inputMask, ORFloatImage_CPtr(view->depth,
  boost::serialization::null_deleter()), -1.0f); maskedDepthImage->UpdateDeviceFromHost();
    view->depth->Swap(*maskedDepthImage);
  }
  */
...
void SLAMComponent::set_detect_fiducials(bool detectFiducials) { m_detectFiducials = detectFiducials; }

void SLAMComponent::set_fusion_enabled(bool fusionEnabled) { m_fusionEnabled = fusionEnabled; }
...
std::vector<SE3Pose> SLAMComponent::load_ground_truth_relocalisation_trajectory() const
{
  ...
  // Detect any sequences for which we're using force-fail tracking, create a disk-based tracker for each of them, and
  // wrap them into a composite tracker.
  ...
  Tracker_Ptr groundTruthTracker =
      m_context->get_tracker_factory().make_tracker_from_string(trackerConfig,
                                                                m_sceneID,
                                                                trackSurfels,
                                                                slamState->get_rgb_image_size(),
                                                                slamState->get_depth_image_size(),
                                                                m_lowLevelEngine,
                                                                m_imuCalibrator,
                                                                settings,
                                                                dummy);
...
void SLAMComponent::prepare_for_tracking(TrackingMode trackingMode)
{
  ...
  case TRACK_SURFELS: {
    const SpaintSurfelScene_Ptr &surfelScene = slamState->get_surfel_scene();
    const SurfelRenderState_Ptr &liveSurfelRenderState = slamState->get_live_surfel_render_state();
    m_trackingController->Prepare(trackingState.get(),
                                  surfelScene.get(),
                                  view.get(),
                                  m_context->get_surfel_visualisation_engine().get(),
                                  liveSurfelRenderState.get());
    break;
  }
  case TRACK_VOXELS:
  default: {
...
void SLAMComponent::process_relocalisation()
{
  ...
  // Relocalise if either (a) the tracking has failed, or (b) we're forcibly relocalising every frame for evaluation
  // purposes.
  const bool performRelocalisation =
      m_relocaliseEveryFrame || trackingState->trackerResult == ITMTrackingState::TRACKING_FAILED;
  if(performRelocalisation)
  {
    std::vector<Relocaliser::Result> relocalisationResults =
        relocaliser->relocalise(view->rgb, view->depth, depthIntrinsics);
    ...
    if(!relocalisationResults.empty())
    {
      ...
      trackingState->trackerResult = bestRelocalisationResult.quality == Relocaliser::RELOCALISATION_GOOD
...
Relocaliser_Ptr SLAMComponent::refine_with_icp(const Relocaliser_Ptr &relocaliser) const
{
  ...
  std::string trackerParams =
      settings->get_first_value<std::string>(m_settingsNamespace + "refinementTrackerParams", "");
  if(trackerParams != "") trackerConfig += "<params>" + trackerParams + "</params>";
  trackerConfig += "</tracker>";

  const bool trackSurfels = false;
  FallibleTracker *dummy;
  Tracker_Ptr tracker = m_context->get_tracker_factory().make_tracker_from_string(trackerConfig,
                                                                                  m_sceneID,
                                                                                  trackSurfels,
                                                                                  rgbImageSize,
                                                                                  depthImageSize,
  ...
  return Relocaliser_Ptr(new ICPRefiningRelocaliser<SpaintVoxel, ITMVoxelIndex>(relocaliser,
                                                                                tracker,
                                                                                rgbImageSize,
                                                                                depthImageSize,
...
void SLAMComponent::setup_fiducial_detector()
{
  ...
  else if(type == "vicon")
  {
    ...
    if(vicon)
    {
      FiducialDetector_CPtr calibratingDetector(
          new ArUcoFiducialDetector(scene, settings, ArUcoFiducialDetector::PEM_RAYCAST));
...
void SLAMComponent::setup_tracker()
{
  ...
  m_tracker = m_context->get_tracker_factory().make_tracker_from_string(m_trackerConfig,
                                                                        m_sceneID,
                                                                        m_trackingMode == TRACK_SURFELS,
                                                                        rgbImageSize,
                                                                        depthImageSize,
                                                                        m_lowLevelEngine,
                                                                        m_imuCalibrator,
